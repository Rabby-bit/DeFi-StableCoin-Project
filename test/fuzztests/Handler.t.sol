...
contract Handler is Test {
    ...
    function redeemCollateral(uint256 tokenSeed, uint256 amountCollateral) public {
        address token = _getTokenAddress(tokenSeed);
        amount = bound(amount, 1, MAX_DEPOSIT_SIZE);
        amountCollateral = bound(amount, 1, MAX_DEPOSIT_SIZE);

        _useActor(tokenSeed);
        dsc_engine.redeemCollateral(token, amountCollateral);
        vm.stopPrank();
    ...
    function burnDSC(uint256 tokenSeed, uint256 amountToBurn) public {
        amountToBurn = bound(amount, 1, MAX_DEPOSIT_SIZE);

        _useActor(tokenSeed);
        dsc_engine.burnDSC(amountToBurn);
        vm.stopPrank();
    }
